...
public class Jarvis
{
    ...
    public async Task RunAsync()
    {
        StartOllama();
        InitializeTTS();
        runner = new AgentRunner(agentConfigs, client);
    ...
    private bool HandleAgentCommand(string command)
    {
       if (runner == null) return false;
        try
        {
            ...
            foreach (var var in config.PromptTemplate
             .Split(new[] { '{', '}' }, StringSplitOptions.RemoveEmptyEntries)
             .Where((_, i) => i % 2 != 0))
            {
                Console.WriteLine($"enter value for {var}");
                inputVars[var] = Console.ReadLine()?.Trim() ?? "";
            }

            var result = runner.RunAgentAsync(agentName).GetAwaiter().GetResult();

            conversationHistory.Add(new Jarvis.Message { Role = "user", Content = command });
            conversationHistory.Add(new Jarvis.Message { Role = "assistant", Content = result });

            Console.WriteLine("\nJarvis: " + result + "\n");
            tts.SpeakAsync(result);
